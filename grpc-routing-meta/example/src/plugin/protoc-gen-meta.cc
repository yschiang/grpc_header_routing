  ...
  bool Generate(const FileDescriptor* file, const std::string&,
...

    {
      ...
      for (int i = 0; i < file->message_type_count(); ++i) {
        ...
        p.Print("void ProjectMeta(const $ns$::$m$& req, routingmeta::MetadataSink& sink);\n",
                "ns", ns, "m", d->name());
      }
    }

    {
      ...
      for (int i = 0; i < file->message_type_count(); ++i) {
        ...
        for (const auto& pj : projs) {
          ...
          if (pj.required) {
            // required: missing -> throw, then emit unconditionally (no dead re-check).
            p.Print("  if ($v$.empty()) throw std::runtime_error(\"$k$ required\");\n",
                    "v", v, "k", pj.key);
            p.Print("  sink.Add(\"$k$\", routingmeta::UrlEncode($v$));\n",
                    "v", v, "k", pj.key);
          } else {
