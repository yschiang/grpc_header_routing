  ...
  for (int j = 0; j < d->field_count(); ++j) {
    ...
    if (f->is_repeated() && f->cpp_type() == FieldDescriptor::CPPTYPE_MESSAGE)
      for (int k = 0; k < f->message_type()->field_count(); ++k)
        if (f->message_type()->field(k)->options().HasExtension(routing::pctx)) return f;
  }
  return nullptr;
}
  ...
  for (int i = 0; i < d->field_count() && ok; ++i) {
    ...
    if (f->is_repeated()) {
      ...
      if (AnyProject(f->message_type(), &seen)) {
        *err = "(routing.project) is set under repeated field \"" + f->name() + "\" in message " +
               d->name() + " — a repeated value cannot project to a single-valued header";
        ok = false;
      }
    } else {
...
bool Validate(const Descriptor* d, std::string* err) {
  ...
  for (const auto& pj : projs)
    if (!keys.insert(pj.key).second) {
      *err = "duplicate (routing.project) key \"" + pj.key + "\" in message " + d->name() +
             " — a single-valued header would be emitted twice";
      return false;
    }
  std::set<const Descriptor*> onpath;
...
class ProjGen : public CodeGenerator {
 public:
  bool Generate(const FileDescriptor* file, const std::string&, GeneratorContext* ctx,
...

    {
      ...
      p.Print(
          ...
          "// ProjectMeta is re-entrant (AD-12/NFR5): reads req, writes ONLY the per-call\n"
          "// sink; holds no shared mutable state, so concurrent calls on distinct sinks\n"
          "// need no locks.\n"
          "#pragma once\n"
          "#include \"$b$.pb.h\"\n"
      ...
      for (int i = 0; i < file->message_type_count(); ++i) {
        ...
        p.Print(
            "routingmeta::ProjResult ProjectMeta(const $ns$::$m$& req, routingmeta::MetadataSink& "
            "sink);\n",
            "ns", ns, "m", d->name());
      }
      p.Print("}  // namespace routingmeta\n");
...

    {
      ...
      p.Print(
          ...
          "// Re-entrant (AD-12/NFR5): reads req, writes ONLY the per-call sink; holds no\n"
          "// shared mutable state, so concurrent calls on distinct sinks need no locks.\n"
          "#include \"$b$.proj.h\"\n"
