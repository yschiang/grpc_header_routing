...
9. **Domain scalar.** `x-mask-id` is projected via `(routing.project)`, reached by a nested-path walk, and `required` — if the source is empty, `ProjectMeta` reports a `MissingRequired` issue in its `ProjResult` (`ok=false`) and emits `x-routing-error: missing:<key>`; it does **not** throw and does **not** emit the empty header. `(routing.project)` is rejected at codegen time unless it sits on a **non-repeated string scalar leaf** (a repeated, message-typed, or non-string tag, or one reached under a repeated field, fails `Validate` loudly — see `tests/negative/`). **Verified-uniform opt-out of the repeated rejection (SPEC §4.1, ADR 0003):** a string scalar that is a *direct* field of exactly *one* repeated message may set `uniform_across_repeated: true` — element[0] is projected and every element is verified to agree; divergence is a **blocking** `Inconsistent` issue (`ok=false`) + `x-routing-error: inconsistent:<key>` with no scalar header; an empty repeated field with `required` is `MissingRequired`. The flag is rejected with no repeated ancestor or below a second repeated level. The 7 KB guard (8) counts the scalar's bytes when contexts follow it, but does not itself bound the scalar's length; domain scalars are short identifiers by contract.
...
- [ ] Digest round-trip (6): sender→receiver `VerifyDigest` ok; modified context ⇒ fail (drift detected).
...
- [ ] Domain scalar (9): nested path reached; empty ⇒ ProjResult MissingRequired issue + x-routing-error, no throw.
- [ ] Uniform projection (9): N agreeing elements ⇒ one header from element[0]; divergence ⇒ Inconsistent issue + `x-routing-error: inconsistent:<key>`, no header; empty batch + required ⇒ MissingRequired.
