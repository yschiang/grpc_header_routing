...
|---|---|---|
| 投影邏輯(by-tx 抽取 + 路徑) | 每個 sender 手寫,21 tx 各一塊 | **codegen 產,0 人寫**(範例輸出 ~460 行) |
| encode / sha256 / digest / overflow 機制 | ~150–230 行(沒共用就每 sender / 每語言重來) | 共用 helper,寫一次(算進下面 src) |
| 一次性平台 src(plugin + helper + 合約) | — | **~675 行**,一次性 |
...
- **Autogen 人寫的只有 src**:一次性平台 ~675 行(plugin + helper + 合約)+ 每系統 ~5 行標籤 + 每 sender 2 行。**投影那 ~460 行是 codegen 產、不算人寫**(它正是 hardcode 要手寫的東西)。
- 所以重點不是「autogen 行數少」(它一次性 src 反而 ~675 行);而是 **autogen 投影 0 人寫、邊際 ≈ 0**;hardcode 是**每 sender × 每語言 ~數百行、且要維護**。sender / 改動越多,差距越大。
...
| &nbsp;&nbsp;代碼量(人寫) | 一個 sender ~250–400 行 | 投影 0 人寫;一次性 src **~675 行** | 輸 ¹ |
| **③ 延展性(本情境重要性較低)** | | | |
| &nbsp;&nbsp;改動 / 維護 + 新增系統 | 每處手動同步、重來 sender + test | 改 proto 重 codegen(+1 proto) | 平 ² |

¹ 1 sender 首次、plugin 從零做:autogen 一次性 src(~675)反而比單一 hardcode sender(~350)**多**。但那是 reusable、投影 0 人寫、0 維護;**plugin 若已存在則此格轉大優**。
...
> [`example/`](example/);wire 規格見 [`SPEC.md`](SPEC.md)。
