...
| sanitizers | grep `-fsanitize`; `command -v leaks valgrind` | ASan/TSan/UBSan wired, else GAP. **LSan unsupported on macOS** → use `leaks --atExit` or defer leak detection to Linux CI |
...
| 9 | **Memory safety** | ASan + UBSan report no error on the test suite; **leak-free** at exit | auto-gate (GAP where a sanitizer is unsupported on the platform) | rebuild tests `-fsanitize=address,undefined` + run (exclude `vptr,function` to drop third-party noise); leaks: `ASAN_OPTIONS=detect_leaks=1` (Linux) / `leaks --atExit` (macOS, since LSan is unsupported there) |
...
- **Trusting clang-tidy/clang-analyzer without a real compilation database.** Missing include paths →
  recovery AST → confident false positives (e.g. "variable not initialized" on initialized vars). Run with
  `compile_commands.json` (cmake export / `bear`) or pass the sysroot; verify **zero `clang-diagnostic-error`**
  before trusting any finding.
