...
int main() {
  ...
  // --- digest gate: accept + every reject form (FR5/AD-9, story 1.11) ---
  {
    ...
    // reject: corrupted digest header, intact body
    {
      ...
      assert(!r.ok);
      assert(r.actual_digest != r.expected_digest);
      assert(r.error.find("mismatch") != std::string::npos);
    }
    ...
    // reject: no digest provided (overflow / sender omitted)
    {
      ...
      assert(!r.ok);
      assert(r.error.find("no digest") != std::string::npos);
    }
  }
  ...
  // --- overflow by COUNT (>25) ---
  {
    ...
    assert(sink.Get("x-process-context-format") ==
           "urlencoded-query-string-v1");  // survives overflow (inv. 8)
  ...
  // --- routingmeta::Send: branchless kit sender = FillCommon + ProjectMeta, no throw ---
  {
    ...
    routingmeta::VectorSink s2;
    auto rb =
        routingmeta::Send(bad, routingmeta::Runtime{"C", "F18", "TOOL1"}, s2);  // must NOT throw
  ...
  assert(routingmeta::UrlDecode(routingmeta::UrlEncode("\xC3\xA9")) ==
         "\xC3\xA9");  // high-byte round-trip
  ...
  {
    routingmeta::VectorSink s;
    sys3::v1::Submit01Request r;
    r.set_mask_id("RET-01");
    assert(ProjectMeta(r, s).ok);
    assert(s.Get("x-mask-id") == "RET-01");
  }  // top-level
  {
    routingmeta::VectorSink s;
    sys3::v1::Submit02Request r;
    r.set_reticle_id("RET-02");
    assert(ProjectMeta(r, s).ok);
    assert(s.Get("x-mask-id") == "RET-02");
  }  // renamed
  {
    routingmeta::VectorSink s;
    sys3::v1::Submit03Request r;
    r.mutable_mask()->set_id("RET-03");
    assert(ProjectMeta(r, s).ok);
    assert(s.Get("x-mask-id") == "RET-03");
  }  // nested 1
  {
    routingmeta::VectorSink s;
    sys3::v1::Submit04Request r;
    r.mutable_layer()->set_mask_no("RET-04");
    assert(ProjectMeta(r, s).ok);
    assert(s.Get("x-mask-id") == "RET-04");
  }  // nested 1, renamed
  {
    routingmeta::VectorSink s;
    sys3::v1::Submit05Request r;
    r.mutable_job()->mutable_mask()->set_mask_id("RET-05");
    assert(ProjectMeta(r, s).ok);
    assert(s.Get("x-mask-id") == "RET-05");
  }  // nested 2
  {
    routingmeta::VectorSink s;
    sys3::v1::Submit06Request r;
    r.mutable_request_header()->set_mask_id("RET-06");
    assert(ProjectMeta(r, s).ok);
    assert(s.Get("x-mask-id") == "RET-06");
  }  // sub-msg
  {
    routingmeta::VectorSink s;
    sys3::v1::Submit07Request r;
    r.set_photo_mask_id("RET-07");
    assert(ProjectMeta(r, s).ok);
    assert(s.Get("x-mask-id") == "RET-07");
  }  // renamed top-level
  {
    routingmeta::VectorSink s;
    sys3::v1::Submit08Request r;
    r.mutable_mask_block()->set_mask_ref("RET-08");
    assert(ProjectMeta(r, s).ok);
    assert(s.Get("x-mask-id") == "RET-08");
  }  // nested
  {
    routingmeta::VectorSink s;
    sys3::v1::Submit09Request r;
    r.set_maskid("RET-09");
    assert(ProjectMeta(r, s).ok);
    assert(s.Get("x-mask-id") == "RET-09");
  }  // top-level
  {
    routingmeta::VectorSink s;
    sys3::v1::Submit10Request r;
    r.mutable_exposure()->mutable_mask()->set_id("RET-10");
    assert(ProjectMeta(r, s).ok);
    assert(s.Get("x-mask-id") == "RET-10");
  }  // nested 2
  // empty-required on a NON-Submit05, deep-nested message -> failure-as-data
  {
    sys3::v1::Submit10Request empty;
    routingmeta::VectorSink s;
  ...
  {
    auto m = routingmeta::ParseContext("=");
    assert(m.size() == 1);
    assert(m[""] == "");
  }
  {
    auto m = routingmeta::ParseContext("=v");
    assert(m.size() == 1);
    assert(m[""] == "v");
  }
  {
    auto m = routingmeta::ParseContext("k=");
    assert(m.size() == 1);
    assert(m["k"] == "");
  }
  ...
  assert(routingmeta::VerifyDigest({"garb=%", "x"}, "sha256:" + std::string(64, '0')).ok == false);
  assert(routingmeta::VerifyDigest({"garb=%"}, "sha256:zz").ok ==
         false);  // malformed digest string -> data, no crash
