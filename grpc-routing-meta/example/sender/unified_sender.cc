...
int main() {
  const routingmeta::Runtime rt{"CORR-LOT01-001", "F18",
                                "ETCH01"};  // [+meta] values you already have
  ...
  // --- sys3  sys3.layout.submit — nested mask id (Submit05), no context ---
  {
    ...
    req.mutable_job()->mutable_mask()->set_mask_id(
        "RET-9981");               // [app] business data (your mask id);
  ...
  // --- sys3 EMPTY required mask — failure-as-data through Send (no throw) ---
  {
    ...
    routingmeta::VectorSink sink;   // [+meta]
    auto r = Send(req, routingmeta::Runtime{"CORR-sys3-005", "F18", "LITHO01"},
                  sink);                                          // [+meta] no throw
  ...
  std::printf(
      "All 16 transaction types (sys1 x1, sys2 x5, sys3 x10) route through the same Send<>().\n");
  return 0;
}
